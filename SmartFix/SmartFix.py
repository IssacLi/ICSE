...
def main():
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="SmartFix 1.0")
    ...
    if args.inference:
        ...
        try:
            ...
            print(repr(e))
            sys.exit(-7)
        cfg_build_time_smartfix = time.time() - cfg_build_start
        dead_basic_blocks = 0
        for basic_block in cfg.basic_blocks:
            if len(basic_block.all_incoming_basic_blocks) == 0 and len(basic_block.all_outgoing_basic_blocks) == 0:
                if not (len(basic_block.instructions) == 1 and basic_block.instructions[0].mnemonic in ["STOP", "INVALID"]):
                    dead_basic_blocks += 1
        cfg_percentage_smartfix = (len(cfg.basic_blocks) - dead_basic_blocks) / len(cfg.basic_blocks) * 100
        print("[SmartFix] Recovered", str(cfg_percentage_smartfix)+"%", "of the control-flow graph in", cfg_build_time_smartfix, "second(s). [SmartFix]")
